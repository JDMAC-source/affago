...
def tower_of_bable(request):
	...
	if request.user.is_authenticated:
		...
		if not loggedinanon.post_sort_depth_char:
			...
			loggedinanon.save()
		startdate = datetime.now() + timedelta(minutes=int(loggedinanon.post_sort_from_date_char.split(',')[0]), hours=int(loggedinanon.post_sort_from_date_char.split(',')[1]), days=int(loggedinanon.post_sort_from_date_char.split(',')[2]), weeks=int(loggedinanon.post_sort_from_date_char.split(',')[3]))
...
def tob_spaces_post(request, space_id, post_id, count):
	...
	if request.user.is_authenticated:
		...
		if loggedinauthor in tob_space.approved_voters.all() or tob_space.public:
			...
			count = int(count)
			startdate = datetime.now() + timedelta(minutes=int(loggedinanon.comment_sort_from_date_char.split(',')[0]), hours=int(loggedinanon.comment_sort_from_date_char.split(',')[1]), days=int(loggedinanon.comment_sort_from_date_char.split(',')[2]), weeks=int(loggedinanon.comment_sort_from_date_char.split(',')[3]))
