...
def base_redirect(request, error):
	if request.COOKIES['current'] == ('tob_view_users' or 'tower_of_bable' or 'tob_dics' or 'word_attributess' or 'mutawords' or 'complementary_scholar') :
		...
		response.set_cookie('error', error)
		return response
	elif request.COOKIES['current'] == 'storefront':
		response = redirect("Bable:"+'storefront', author=request.COOKIES['author'], dictionary_id=request.COOKIES['dictionary'], storefront_title=request.COOKIES['storefront'])
		response.set_cookie('error', error)
		return response
...
def tower_of_bable(request):
	...
	sponsor, x = Sponsor.objects.get_or_create(allowable_expenditure=1000000000000, the_sponsorship_phrase="If S is Alpha S is G", img="https://www.predictionary.us/static/bullseye_3.jpeg", url2="https://www.predictionary.us/user/test/space/iverdabefore/count/0/", price_limit=1, author=test)
	...
	if request.user.is_authenticated:
		...
		the_response = render(request, 'tower_of_bable.html', {"post_filter_depth_form": post_filter_depth_form, "post_filter_from_date_form": post_filter_from_date_form,"basic_price": basic_price, "post_sort_form": post_sort_form, "postscount": postscount, "ip": ip, "x_forwarded_for": x_forwarded_for, "buyadvertisingform": buyadvertisingform, "file_form": file_form, "total": total, "count": lower, "mcount": mcount, "count100": count100, "loggedinanon": loggedinanon, "posts": posts_by_viewcount, 'loginform': loginform, 'registerform': registerform,  'word_form': word_form, 'dic_form':dic_form, 'space_form': space_form, "post_form": post_form, 'task_form': task_form, 
			"apply_votestyle_form": apply_votestyle_form, "create_votes_form": create_votes_form, "exclude_votes_form": exclude_votes_form, "apply_dic_form": apply_dic_form, "exclude_dic_form": exclude_dic_form})
	else:
		previous_view = UserViews.objects.filter(ip_address=ip).order_by('-view_date').first()
		pages_view = UserViews.objects.create(page_view="tower_of_bable", ip_address=ip, httpxforwardfor=x_forwarded_for)
		page_views.user_views.add(pages_view)
		if previous_view:
			pages_view.previous_view_id = previous_view.id
			pages_view.previous_page = previous_view.page_view
...
def tob_view_spaces(request):
	...
	page_views, created = Pageviews.objects.get_or_create(page="tob_views_spaces")
	page_views.views += 1
	page_views.save()
	...
	if request.user.is_authenticated:
		...
		previous_view = UserViews.objects.filter(ip_address=ip).order_by('-view_date').first()
		pages_view = UserViews.objects.create(page_view="tob_view_spaces", ip_address=ip, httpxforwardfor=x_forwarded_for)
		page_views.user_views.add(pages_view)
		if previous_view:
			pages_view.previous_view_id = previous_view.id
			pages_view.previous_page = previous_view.page_view
...
def tob_view_spaces_count(request, count):
	...
	page_views, created = Pageviews.objects.get_or_create(page="tob_view_saces_count")
	page_views.views += 1
	page_views.save()
	loginform = AuthenticationForm()
	if request.user.is_authenticated:
		...
		previous_view = UserViews.objects.filter(ip_address=ip).order_by('-view_date').first()
		pages_view = UserViews.objects.create(page_view="tob_view_spaces_count__"+str(count), ip_address=ip, httpxforwardfor=x_forwarded_for)
		page_views.user_views.add(pages_view)
		if previous_view:
			pages_view.previous_view_id = previous_view.id
			pages_view.previous_page = previous_view.page_view
...
def tob_space_view(request, space):
	...
	page_views, created = Pageviews.objects.get_or_create(page="tob_space_view")
	page_views.views += 1
	page_views.save()
	loginform = AuthenticationForm()
	if request.user.is_authenticated:
	...
	if request.user.is_authenticated:
		...
		previous_view = UserViews.objects.filter(ip_address=ip).order_by('-view_date').first()
		pages_view = UserViews.objects.create(page_view="tob_space_view__"+str(space), ip_address=ip, httpxforwardfor=x_forwarded_for)
		page_views.user_views.add(pages_view)
		if previous_view:
			pages_view.previous_view_id = previous_view.id
			pages_view.previous_page = previous_view.page_view
...
def tob_space_view_count(request, space, count):
	...
	page_views, created = Pageviews.objects.get_or_create(page="tob_space_view_count")
	page_views.views += 1
	page_views.save()
	loginform = AuthenticationForm()
	if request.user.is_authenticated:
			...
			if request.user.is_authenticated:
				if not users_space.public:
					if request.user.username in users_space.approved_voters.all().values_list('username'):
						...
						return HttpResponse("Not Invited To Space")
				else:
					spaces_posts = spaces_posts.order_by(loggedinanon.post_sort_char)[count:count+100]
					space_viewable = True
			else:
				if users_space.public:
					...
					return HttpResponse("Space not public")
		else:
			if users_space.public:
				space_viewable = True
			else:
	...
	if request.user.is_authenticated:
		...
		previous_view = UserViews.objects.filter(ip_address=ip).order_by('-view_date').first()
		pages_view = UserViews.objects.create(page_view="tob_space_view_count__"+str(space)+"__"+str(count), ip_address=ip, httpxforwardfor=x_forwarded_for)
		page_views.user_views.add(pages_view)
		if previous_view:
			pages_view.previous_view_id = previous_view.id
			pages_view.previous_page = previous_view.page_view
...
def tob_sponsor_product(request, product_id):
	...
	if request.user.is_authenticated:
		...
		previous_view = UserViews.objects.filter(ip_address=ip).order_by('-view_date').first()
		pages_view = UserViews.objects.create(page_view="tob_sponsor_product__"+product_id, ip_address=ip, httpxforwardfor=x_forwarded_for)
		page_views.user_views.add(pages_view)
		if previous_view:
			pages_view.previous_view_id = previous_view.id
			pages_view.previous_page = previous_view.page_view
...
def tob_product(request, product_id):
	...
	if request.user.is_authenticated:
		...
		previous_view = UserViews.objects.filter(anon=loggedinanon).order_by('-view_date').first()
		pages_view = UserViews.objects.create(page_view="tob_product__"+product_id, anon=loggedinanon, ip_address=ip, httpxforwardfor=x_forwarded_for)
		page_views.user_views.add(pages_view)
		if previous_view:
			pages_view.previous_view_id = previous_view.id
			pages_view.previous_page = previous_view.page_view
		...
		previous_view = UserViews.objects.filter(ip_address=ip).order_by('-view_date').first()
		pages_view = UserViews.objects.create(page_view="tob_product__"+product_id, ip_address=ip, httpxforwardfor=x_forwarded_for)
		page_views.user_views.add(pages_view)
		if previous_view:
			pages_view.previous_view_id = previous_view.id
			pages_view.previous_page = previous_view.page_view
...
def tob_products(request, count):
	...
	products_count = Price.objects.all().count()
	...
	total = Pageviews.objects.all().count()
	...
	if request.user.is_authenticated:
		...
		file_form = FileForm() 
		the_response = render(request, "tob_products.html", {"products_count":products_count, "ip": ip, "x_forwarded_for": x_forwarded_for, "file_form": file_form, "loggedinanon": loggedinanon, "products": products, "space_form": space_form, "post_form": post_form, "task_form": task_form, "word_form": word_form, "registerform": registerform,  "loginform": loginform, 
			"apply_votestyle_form": apply_votestyle_form, "create_votes_form": create_votes_form, "exclude_votes_form": exclude_votes_form, "apply_dic_form": apply_dic_form, "exclude_dic_form": exclude_dic_form})
	else:
		previous_view = UserViews.objects.filter(ip_address=ip).order_by('-view_date').first()
		pages_view = UserViews.objects.create(page_view="tob_products__"+str(count), ip_address=ip, httpxforwardfor=x_forwarded_for)
		page_views.user_views.add(pages_view)
		if previous_view:
			pages_view.previous_view_id = previous_view.id
			pages_view.previous_page = previous_view.page_view
			pages_view.previous_view_date = previous_view.view_date
			pages_view.previous_view_time_between_pages = datetime.datetime.now(timezone.utc) - previous_view.view_date
		products = Price.objects.all()[count:count+100]
		the_response = render(request, "tob_products.html", {"products_count":products_count, "ip": ip, "x_forwarded_for": x_forwarded_for, "products": products, "registerform": registerform,  "loginform": loginform})
...
def storefronts(request, count):
	...
	if request.user.is_authenticated:
		...
		storefront_dic_form = StorefrontDicForm(user_anon=loggedinanon)
		...
		file_form = FileForm() 
		the_response = render(request, "tob_storefronts.html", {"ip": ip, "x_forwarded_for": x_forwarded_for, "file_form": file_form, "loggedinanon": loggedinanon, "storefronts": storefronts, "storefronts_count": storefronts_count, "storefront_sort_form": storefront_sort_form, "storefront_dic_form": storefront_dic_form, "space_form": space_form, "post_form": post_form, "task_form": task_form, "word_form": word_form, "registerform": registerform,  "loginform": loginform, 
			"apply_votestyle_form": apply_votestyle_form, "create_votes_form": create_votes_form, "exclude_votes_form": exclude_votes_form, "apply_dic_form": apply_dic_form, "exclude_dic_form": exclude_dic_form})
	else:
		previous_view = UserViews.objects.filter(ip_address=ip).order_by('-view_date').first()
		pages_view = UserViews.objects.create(page_view="storefronts__"+str(count), ip_address=ip, httpxforwardfor=x_forwarded_for)
		page_views.user_views.add(pages_view)
		if previous_view:
			pages_view.previous_view_id = previous_view.id
			pages_view.previous_page = previous_view.page_view
...
def users_storefronts(request, user, count):
	...
	if request.user.is_authenticated:
		...
		previous_view = UserViews.objects.filter(ip_address=ip).order_by('-view_date').first()
		pages_view = UserViews.objects.create(page_view="user_storefronts__"+user+"__"+str(count), ip_address=ip, httpxforwardfor=x_forwarded_for)
		page_views.user_views.add(pages_view)
		if previous_view:
			pages_view.previous_view_id = previous_view.id
			pages_view.previous_page = previous_view.page_view
...
def create_storefront(request, dictionary):
	if request.method=="POST":
		...
		storefront = Storefront.objects.create(author=Author.objects.get(username=request.user.username), logo=word, title=request.POST.get("title"), preview_text=request.POST.get("preview_text"), disclaimer=request.POST.get("disclaimer"), image_1=request.POST.get("image_1"), image_2=request.POST.get("image_2"), image_3=request.POST.get("image_3"), image_4=request.POST.get("image_4"), image_5=request.POST.get("image_5"), template_section_size_title_left=request.POST.get("template_section_size_title_left"), template_section_size_title_right=request.POST.get("template_section_size_title_right"), template_section_size_title_height=request.POST.get("template_section_size_title_height"), template_section_size_preview_left=request.POST.get("template_section_size_preview_left"), template_section_size_preview_right=request.POST.get("template_section_size_preview_right"), template_section_size_preview_height=request.POST.get("template_section_size_preview_height"), template_section_size_disclaimer_left=request.POST.get("template_section_size_disclaimer_left"), template_section_size_disclaimer_right=request.POST.get("template_section_size_disclaimer_right"), template_section_size_disclaimer_height=request.POST.get("template_section_size_disclaimer_height"), textblock_1=request.POST.get("textblock_1"), textblock_2=request.POST.get("textblock_2"), textblock_3=request.POST.get("textblock_3"), textblock_4=request.POST.get("textblock_4"))
		loggedinanon.storefronts.add(storefront)
		loggedinanon.save()
		dictionary__.storefronts.add(storefront)
		dictionary__.save()
...
def tob_spaces_post(request, space_id, post_id, count):
	registerform = UserCreationForm()
	...
	if request.user.is_authenticated:
		...
		previous_view = UserViews.objects.filter(anon=loggedinanon).order_by('-view_date').first()
		pages_view = UserViews.objects.create(page_view="tob_spaces_post__"+space_id+"__"+post_id, anon=loggedinanon, ip_address=ip, httpxforwardfor=x_forwarded_for)
		page_views.user_views.add(pages_view)
		if previous_view:
			pages_view.previous_view_id = previous_view.id
			pages_view.previous_page = previous_view.page_view
		...
		tob_space = Space.objects.filter(id=int(space_id)).first()

		if loggedinauthor in tob_space.approved_voters.all() or tob_space.public:
			dic_form = DictionaryForm()
			post_form = PostForm(request)
			space_form = SpaceForm(request)
			task_form = TaskForm()
			...
			the_response.set_cookie('space', int(space_id))
			the_response.set_cookie('post', int(post_id))
			the_response.set_cookie('count', count)
			return the_response
		else:
			return redirect("Bable:tob_space_view_count", space_id, 0)
	elif tob_space.public:
		previous_view = UserViews.objects.filter(ip_address=ip).order_by('-view_date').first()
		pages_view = UserViews.objects.create(page_view="tob_spaces_post__"+space_id+"__"+post_id, ip_address=ip, httpxforwardfor=x_forwarded_for)
		page_views.user_views.add(pages_view)
		if previous_view:
			pages_view.previous_view_id = previous_view.id
			pages_view.previous_page = previous_view.page_view
		...
		tob_post = tob_space.posts.filter(id=int(post_id)).first()
		count = int(count)
		tob_comments = tob_post.comments.all()[count:count+100]
		
		the_response = render(request, "tob_spaces_post.html", {"tob_space": tob_space, "tob_post": tob_post, "tob_comments": tob_comments, "registerform": registerform,  "loginform": loginform})
		the_response.set_cookie('current', 'tob_spaces_post')
		the_response.set_cookie('space', int(space_id))
		the_response.set_cookie('post', int(post_id))
		the_response.set_cookie('count', count)
	...
	the_response.set_cookie('current', 'tob_space')
	the_response.set_cookie('space', space)
	return base_redirect(request, 0)


def tob_spaces_posts_comment(request, space_id, post_id, comment_id):
	user_themself = User.objects.get(username=user)
	user_anon = Anon.objects.get(username=user_themself)
	...
	tob_space = Space.objects.filter(id=space_id).first()
	...
	if request.user.is_authenticated:
		...
		if loggedinauthor in tob_space.approved_voters.all() or tob_space.public:
			...
			tob_comments = tob_comment.children().filter(latest_change_date__range=[enddate, startdate]).order_by(loggedinanon.comment_sort_char)
			...
			the_response.set_cookie('space', int(space_id))
			the_response.set_cookie('post', int(post_id))
			the_response.set_cookie('comment', int(comment_id))
			return the_response
		else:
			return redirect('Bable:tob_space_view_count', space_id, 0)
	elif tob_space.public:
		...
		count = int(count)
		tob_comment = tob_post.comments.filter(id=comment_id).first()
		tob_comments = tob_comment.children()
		
		the_response = render(request, "tob_spaces_post.html", {"tob_space": tob_space, "tob_post": tob_post, "tob_comment": tob_comment, "tob_comments": tob_comments, "registerform": registerform,  "loginform": loginform})
		the_response.set_cookie('current', 'tob_spaces_post')
		the_response.set_cookie('space', int(space_id))
		the_response.set_cookie('post', int(post_id))
		the_response.set_cookie('count', count)
		return the_response
	return redirect('Bable:tob_space_view_count', space_id)
...
def tob_sponsor(request, sponsor):
	...
	if request.user.is_authenticated:
		...
		previous_view = UserViews.objects.filter(anon=loggedinanon).order_by('-view_date').first()
		pages_view = UserViews.objects.create(page_view="tob_sponsor__"+int(sponsor), anon=loggedinanon, ip_address=ip, httpxforwardfor=x_forwarded_for)
		page_views.user_views.add(pages_view)
		if previous_view:
			pages_view.previous_view_id = previous_view.id
			pages_view.previous_page = previous_view.page_view
		...
		previous_view = UserViews.objects.filter(ip_address=ip).order_by('-view_date').first()
		pages_view = UserViews.objects.create(page_view="tob_sponsor__"+int(sponsor), ip_address=ip, httpxforwardfor=x_forwarded_for)
		page_views.user_views.add(pages_view)
		if previous_view:
			pages_view.previous_view_id = previous_view.id
			pages_view.previous_page = previous_view.page_view
	...
	if not users_space.public:
		if request.user.is_authenticated:
			if len(users_space.filter(approved_voters=loggedinauthor))>0:
				...
				posts_comments = spaces_post.comments.order_by(loggedinanon.comment_sort_char).filter()[0:100]
...
def tob_users_dic(request, user, dictionary, count):
	...
	if request.user.is_authenticated:
		...
		if users_dic.public:
			dics_words = users_dic.words.order_by('the_word_itself')[count:count+100]
			...
			the_response = render(request, "tob_users_dic.html", {"user_anon": user_anon, "users_dic": users_dic, "registerform": registerform,  "loginform": loginform})
...
def storefront(request, author, dictionary_id, storefront_title):
	...
	the_response.set_cookie('storefront', storefront_title)
	return the_response

def storefront_add_product(request, author, storefront_title):
	user_anon = Anon.objects.get(username=User.objects.get(username=author))
	user_storefront = user_anon.storefronts.filter(title=storefront_title).first()
	if user_storefront.products.count():
		user_products = user_storefront.products.all()
		...
		if author == request.user.username:
			if request.method == "POST":
				...
				if product_form.is_valid():
					...
					loggedinanon.products.add(product)
					loggedinanon.save()
					if user_storefront in loggedinanon.storefronts.all():
						user_storefront.products.add(product)
						user_storefront.save()
	return base_redirect(request, 0)	
	



def storefront_landing_page(request, author, storefront_title):
	user_anon = Anon.objects.get(username=User.objects.get(username=author))
	user_storefront = user_anon.storefronts.filter(title=storefront_title).first()
	if user_storefront.products.count():
		user_products = user_storefront.products.all()
	...
	sale_form = SaleForm()
	page_views, created = Pageviews.objects.get_or_create(page="storefront_landing_page")
	page_views.views += 1
	page_views.save()	
...
def storefront_index(request, author, storefront_title):
	user_anon = Anon.objects.get(username=User.objects.get(username=author))
	user_storefront = user_anon.storefronts.filter(title=storefront_title).first()
	if user_storefront.products.count():
		user_products = user_storefront.products.all()
	...
	sale_form = SaleForm()
	page_views, created = Pageviews.objects.get_or_create(page="storefront_index")
	page_views.views += 1
	page_views.save()	
...
def storefront_about(request, author, storefront_title):
	...
	sale_form = SaleForm()
	page_views, created = Pageviews.objects.get_or_create(page="storefront_about")
	page_views.views += 1
	page_views.save()	
...
def storefront_product_list(request, author, dictionary_id, storefront_title):
	...
	sale_form = SaleForm()
	page_views, created = Pageviews.objects.get_or_create(page="storefront_product_list")
	page_views.views += 1
	page_views.save()	
	if request.user.is_authenticated:
		loggedinanon = Anon.objects.get(username=request.user)
...
def storefront_past_purchases(request, author, dictionary_id, storefront_title):
	...
	sale_form = SaleForm()
	page_views, created = Pageviews.objects.get_or_create(page="storefront_past_purchase")
	page_views.views += 1
	page_views.save()	
...
def clickthrough(request):
	...
	if request.method == "POST":
		...
		if request.user.is_authenticated:
			...
			previous_view = UserViews.objects.filter(anon=loggedinanon).order_by('-view_date').first()
			pages_view = UserViews.objects.create(page_view="clickthrough__sponsor_id__"+str(sponsor_id)+"__ip__"+request.META.get('REMOTE_ADDR'), anon=loggedinanon, ip_address=ip, httpxforwardfor=x_forwarded_for)
			page_views.user_views.add(pages_view)
			if previous_view:
				pages_view.previous_view_id = previous_view.id
				pages_view.previous_page = previous_view.page_view
			...
			previous_view = UserViews.objects.filter(ip_address=ip).order_by('-view_date').first()
			pages_view = UserViews.objects.create(page_view="clickthrough__sponsor_id__"+str(sponsor_id)+"__ip__"+request.META.get('REMOTE_ADDR'), ip_address=ip, httpxforwardfor=x_forwarded_for)
			page_views.user_views.add(pages_view)
			if previous_view:
				pages_view.previous_view_id = previous_view.id
				pages_view.previous_page = previous_view.page_view
